...
echo "[run_truth] $(date '+%F %T') start" >>"$LOG"

# Try multiple passes to ensure we cover last 24h.
# (The script upserts & dedupes, so reruns are safe.)
TRUTH_MAX_TOTAL=600 sh ./safe_run.sh python3 truth_pull_public.py  >>"$LOG" 2>&1 || true
TRUTH_MAX_TOTAL=600 sh ./safe_run.sh python3 truth_scrape_html.py  >>"$LOG" 2>&1 || true

# Summarize + tag + echo/contradictions + reports, then rebuild feed
sh ./safe_run.sh python3 news_summarize.py >>"$LOG" 2>&1 || true
sh ./post_ingest.sh                      >>"$LOG" 2>&1 || true

# Rebuild the feed page (already stores full bodies in doc_pages/docs)
