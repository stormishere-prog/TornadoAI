...
sh ./daily_digest.sh >>"$LOG" 2>&1 || true
sh ./daily_digest.sh >>"$LOG" 2>&1 || true
sh ./alerts_zip.sh   >>"$LOG" 2>&1 || true
...
sh ./post_ingest.sh >>"$LOG" 2>python3 fetch_and_ingest.py >>"$LOG" 2>&1 || true1 || true
  # weekly retention: keep last 90 days, protect official sources
  sh ./safe_run.sh sh ./cleanup_retention.sh 90 >>"$LOG" 2>&1 || true
sh ./daily_digest.sh >>"$LOG" 2>&1 || true
sh ./alerts_zip.sh >>"$LOG" 2>&1 || true
sh ./db_guard.sh >>"$LOG" 2>&1 || true
...
sh ./safe_run.sh python3 standing_queries_scan.py --max-per-tag 1 --alerts alerts >>"$LOG" 2>&1 || true
    ...
    sleep 3600
    sh ./rotate_logs.sh >>"$LOG" 2>&1 || true
  else
