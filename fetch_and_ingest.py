...
# --- archive freshly ingested PDFs to SD card ---
ARCHIVE_DIR = "/storage/6F3A-4D77/Documents"
def _move_to_archive(pdf_path:str):
    try:
        ...
        # leave a symlink where it used to be, so the rest of the pipeline still resolves paths
        try:
            os.symlink(dst, pdf_path)
        except Exception:
            pass
        print(f"[archive] moved {pdf_path} -> {dst}")
    except Exception as e:
        print(f"[archive] failed for {pdf_path}: {e}")
...
def pdf_pages_to_text(pdf_path:str)->list[str]:
    real_path = resolve_pdf(pdf_path)
    # use pdftotext page by page
    out=[]
    try:
        # get page count
        pinfo=subprocess.check_output(["pdfinfo", real_path], timeout=20, stderr=subprocess.DEVNULL).decode("utf-8","ignore")
        ...
        for p in range(1, pages+1):
            with tempfile.NamedTemporaryFile(delete=False, suffix=".txt") as tf:
                tfp=tf.name
            try:
                subprocess.run(["pdftotext","-enc","UTF-8","-f",str(p),"-l",str(p),real_path,tfp],
        ...
        # fallback: whole doc
        try:
            full=subprocess.check_output(["pdftotext","-enc","UTF-8", real_path, "-"], timeout=40).decode("utf-8","ignore")
...
def handle_url(c, url:str)->int:
    ...
    if is_pdf_bytes(b):
        ...
        os.makedirs("/storage/6F3A-4D77/Documents/TornadoAI/pdfs", exist_ok=True)
        pdf_path_sd    = os.path.join("/storage/6F3A-4D77/Documents/TornadoAI/pdfs", f"{h}.pdf")
        ...
        clear_pages(c, url)
        for i, txt in enumerate(pages or [], start=1):
            insert_page(c, url, i, txt)
        upsert_summary(c, url, short_summary(pages or []))
        added = 1
    else:
        # HTML
        t = text_from_html(b)
        m = re.search(r"<title>(.*?)</title>", b.decode("utf-8","ignore"), re.I|re.S)
        ttl = (m.group(1).strip() if m else url)
